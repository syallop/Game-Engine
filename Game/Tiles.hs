...
mkTiles rows tileset size
  = Just $ Tiles{_tilesRows     = rows
                ,_tilesTileSet  = tileset
                ,_tilesUnitSize = size
                }
...
renderTiles offset (screenWidth,screenHeight) renderer tiles = do
    let tileUnitSize = _tilesUnitSize tiles
    -- TODO:
    ...
    foldM_ (\y row
             -> do foldM_ (\x tiletype
                            -> case M.lookup tiletype (_tilesTileSet tiles) of
                                   Nothing
...
posToTileIx :: Point V2 CInt -> Tiles t -> Maybe TilesIndex
posToTileIx (P (V2 x y)) ts =
  let unitSize = _tilesUnitSize ts
...
indexInRange (V2 xIx yIx) ts = (0 <= xIx) && (xIx < (tilesWidth . _tilesRows $ ts))
                            && (0 <= yIx) && (yIx < (tilesHeight . _tilesRows $ ts))
...
isSolidAt ix ts =
  let minfo = indexTileInfo ix ts
     in maybe False _tileInfoIsSolid minfo
...
indexTileInfo (V2 xIx yIx) ts = do
  tileRow <- indexTileRows (_tilesRows ts) yIx
  t       <- indexTileRow tileRow xIx
  M.lookup t (_tilesTileSet ts)
